...
// Date/Time Elements
    ...
    _Timestamp_: one of
            ...
            DATE _DateTimeSep_ CLOCK SPACE_SEP
    ...
    SPACE_SEP:
            <space>+
    // NOTE: recognised only after a DATE or a CLOCK.  Elsewhere <space>+ is
    //       MULTIPLY, so the scanner decides by context which one it is.
    ...
    // NOTE: a separator is required before a CLOCK.  ISO 8601 permits omitting
    //       it only where no confusion can arise; here a bare digit run after
    //       the shift operator is also a valid numeric origin, so the omission
    //       is not safe.
